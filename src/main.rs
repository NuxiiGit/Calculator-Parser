...
fn main() {
    ...
    parser.add_op(Operator::new("true",  9, |_| 1.0));
    parser.add_op(Operator::new("false", 9, |_| 0.0));
    parser.add_op(Operator::new("(_)",   8, |args| args[0]));
    parser.add_op(Operator::new("|_|",   8, |args| if args[0] >= 0.0 {args[0]} else {-args[0]}));
    parser.add_op(Operator::new("_^_",   7, |args| args[0].powf(args[1])));
